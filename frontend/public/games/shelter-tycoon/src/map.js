...
export function getCorridorTiles() {
  ...
  for (let y = 0; y < MAP_H; y++) {
    for (let x = 0; x < MAP_W; x++) {
      if (tileMap[y][x] === 1) corridors.push({ x, y });
    }
  }
  return corridors;
}
...
export function restoreRoomFromSave(savedRoom) {
  ...
  if (room && savedRoom.constructionProgress != null) {
    room.constructionProgress = savedRoom.constructionProgress;
    room.constructionTick = savedRoom.constructionProgress * room.constructionTime;
  }
  return room;
}
...
/** Re-scan doors after placement or when corridors are painted nearby. */
export function connectRoomToCorridor(room) {
  if (room?.id != null) refreshDoors(room.id);
}
