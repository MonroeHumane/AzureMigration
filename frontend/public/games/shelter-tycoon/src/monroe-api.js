...
export async function syncAdoptedexProfile(slug) {
  ...
  try {
    ...
    if (met.length) {
      primeDiscoveredPetIds(met);
      // Adoptédex "met" ≠ adopted out in this shelter run - do not block intake.
...
export async function loadMonroePets(options = {}) {
  const { logStatus = true } = options;
  ...
  G.shelterCatalog = rawPets
    ...
    .filter(Boolean);
  refreshAvailablePets();
  if (logStatus) logIntakeCatalogStatus();
  return G.availablePets.length;
}

export function logIntakeCatalogStatus() {
  const blocked = getIntakeBlockedPetIds().size;
  const inCare = (G.agents || []).filter((a) => a.petId != null).length;
  console.log(
    `Loaded ${G.shelterCatalog.length} total pets. ${G.availablePets.length} available for intake.`
    + (blocked ? ` (${inCare} in care, ${blocked} blocked)` : ''),
  );
}

/**
 * Older builds merged all Adoptédex met_ids into adoptedPetIds, which blocked every intake.
 * Detect when nearly the whole catalog appears in that list (regardless of adoptedCount).
 */
export function isAdoptedPetIdsPolluted(ids = G.adoptedPetIds) {
  const catalog = G.shelterCatalog || [];
  const catalogSize = catalog.length;
  if (catalogSize === 0) return false;

  const catalogIds = new Set(catalog.map((p) => p.id));
  const list = (ids || []).map(String);
  const overlap = list.filter((id) => catalogIds.has(id)).length;
  return overlap >= Math.max(10, catalogSize - 2);
}

/** Pets adopted out of this shelter run (not Adoptédex "met"). */
export function getShelterReleasedPetIds() {
  const inCare = new Set(
    (G.agents || [])
      .filter((a) => a.petId != null)
      .map((a) => String(a.petId)),
  );
  const fromLog = (G.completedLog || [])
    .filter((e) => e.source === 'adoption' && e.petId != null && e.petId !== '')
    .map((e) => String(e.petId));
  const legacy = (G.adoptedPetIds || []).map(String).filter((id) => !inCare.has(id));

  if (isAdoptedPetIdsPolluted(legacy)) {
    return [...new Set(fromLog)];
  }
  return [...new Set([...fromLog, ...legacy])];
}

/** Block intake only for pets in care or adopted out in this shelter - not dex discoveries. */
export function getIntakeBlockedPetIds() {
  const blocked = new Set();
  for (const agent of G.agents || []) {
    if (agent.petId != null) blocked.add(String(agent.petId));
  }
  for (const entry of G.completedLog || []) {
    if (entry.source === 'adoption' && entry.petId != null && entry.petId !== '') {
      blocked.add(String(entry.petId));
    }
  }
  // Never block intake from adoptedPetIds alone - that list was polluted by old Adoptédex met_ids merges.
  return blocked;
}

export function sanitizeAdoptedPetIds() {
  const before = (G.adoptedPetIds || []).length;
  const polluted = isAdoptedPetIdsPolluted();
  const cleaned = getShelterReleasedPetIds();

  if (!polluted && cleaned.length === before) return false;

  if (polluted && before > cleaned.length) {
    console.warn(
      `[Shelter Tycoon] Cleared ${before - cleaned.length} Adoptédex discovery id(s) from intake block list `
      + `(${before} → ${cleaned.length} shelter adoption id(s)).`,
    );
  }
  G.adoptedPetIds = cleaned;
  return polluted;
}

export function refreshAvailablePets() {
  sanitizeAdoptedPetIds();
  const blocked = getIntakeBlockedPetIds();
  G.availablePets = (G.shelterCatalog || []).filter((pet) => !blocked.has(pet.id));
}
...
// Batched Adoptédex discovers - avoids 429s when many pets leave the shelter quickly.
const dexSyncedPetIds = new Set();
const dexPendingPetIds = new Set();
let dexFlushTimer = null;
let dexFlushSlug = '';
const DEX_DISCOVER_FLUSH_MS = 2500;
const DEX_DISCOVER_RETRY_MS = 12000;

export function primeDiscoveredPetIds(ids) {
  for (const id of ids || []) {
    dexSyncedPetIds.add(String(id));
  }
}

export function discoverPetToDex(slug, petId) {
  if (!slug || petId == null || petId === '') return;
  const id = String(petId);
  if (dexSyncedPetIds.has(id)) return;
  dexPendingPetIds.add(id);
  dexFlushSlug = slug;
  if (!dexFlushTimer) {
    dexFlushTimer = setTimeout(() => {
      dexFlushTimer = null;
      void flushDiscoverQueue();
    }, DEX_DISCOVER_FLUSH_MS);
  }
}

async function flushDiscoverQueue() {
  const slug = dexFlushSlug;
  if (!slug || dexPendingPetIds.size === 0) return;

  const ids = [...dexPendingPetIds];
  ...
  try {
    const res = await fetch(apiUrl(`adoptedex/${encodeURIComponent(slug)}/discover/bulk`), {
      ...
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ pet_ids: ids, source: 'tycoon' }),
    });
    if (res.status === 429) {
      ids.forEach((id) => dexPendingPetIds.add(id));
      dexFlushTimer = setTimeout(() => {
        dexFlushTimer = null;
        void flushDiscoverQueue();
      }, DEX_DISCOVER_RETRY_MS);
      return;
    }
    if (res.ok) {
      ...
      for (const id of [...added, ...skipped]) {
        dexSyncedPetIds.add(String(id));
      }
      return;
    }
  } catch (error) {
    console.warn('Adoptédex bulk discover failed.', error);
  }
  ...
  for (const id of ids) {
    ...
    try {
      const res = await fetch(apiUrl(`adoptedex/${encodeURIComponent(slug)}/discover`), {
        ...
        credentials: 'same-origin',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pet_id: id, source: 'tycoon' }),
      });
      if (res.ok) {
        dexSyncedPetIds.add(id);
        continue;
      }
      if (res.status === 429) {
        dexPendingPetIds.add(id);
      ...
      console.warn('Adoptédex discover failed.', error);
    }
  }
}
...
export async function onPetAdoptedOut(petId, slug) {
  const id = String(petId);
  if (!G.adoptedPetIds.map(String).includes(id)) {
    G.adoptedPetIds.push(id);
  }
  refreshAvailablePets();

  if (!slug) return;
  discoverPetToDex(slug, petId);
}
...
export function serializeGameState() {
  return {
    ...
    day: G.day,
    week: G.week,
    adoptedPetIds: getShelterReleasedPetIds(),
