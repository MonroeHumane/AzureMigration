// Azure arcade API client — anonymous session, scores, cloud save sync, Adoptédex economy.
// Uses the same endpoints as the rest of the arcade (see arcade/public/index.php).
...
// ── Cloud save (sync) ───────────────────────────────────────────────────────
// Save shape: { best, gamesPlayed, totalScore, catId, claimedMilestones: [] }
...
function opId() {
  return 'fc2-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 9);
}
...
export async function pushCloudSave(local) {
  ...
  if (res && res.status === 'success') {
    ...
    // Server wins; adopt its revision+data and merge our best upward next push.
...
// ── Adoptédex economy (milestones → packs/coins, pack opening) ─────────────

export async function claimMilestone(rewardKey) {
  const user = dexUser();
  if (!user) return null;
  await ensureSession();
  if (typeof MonroeAdoptedex !== 'undefined' && MonroeAdoptedex.claimReward) {
    return MonroeAdoptedex.claimReward(apiBase(), user, GAME_ID, rewardKey, { tier: 'standard', count: 1 });
  }
