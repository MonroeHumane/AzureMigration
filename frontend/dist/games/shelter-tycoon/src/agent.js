...
export function getNextAgentId() { return nextCharId; }

export function setNextAgentId(id) {
  nextCharId = Math.max(0, id);
}
...
export class Agent {
  constructor(roleKey, x, y, profile = null) {
    ...
    // Shelter pet identity (Monroe catalog)
    this.petId = profile?.petId ?? null;
    this.species = profile?.species ?? null;
    this.age = profile?.age ?? null;
    this.readyForAdoption = profile?.readyForAdoption ?? false;
