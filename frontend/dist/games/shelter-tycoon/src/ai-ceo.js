...
import { intakeRandomPet, canIntakePet, peekNextIntakePet, rollCandidatesForRole, getCandidatesForRole } from './intake.js';
import { getRoomUnlockState, isOfficeAvailable, chooseTechBranch } from './progression.js';
import { showToast } from './ui/toast.js';
import { startTeamBuilding, escortReadyPetsToLobby } from './simulation.js';
...
const BUILD_PRIORITIES = {
  ...
  animal_shelter:  ['kennel', 'cattery', 'playyard', 'vetbay', 'breakroom'],
};
...
export function aiCeoTick(dt) {
  ...
  if (G.companyType === 'animal_shelter') {
    if (handleShelterAdoptions()) return;
    if (handleShelterIntake()) return;
...
// ─── Intake pets (shelter) or hire agents (business) ─────
function handleHiring() {
  if (G.companyType === 'animal_shelter') {
    return handleShelterIntake();
  }
...
function handleShelterAdoptions() {
  const ready = G.agents.filter((a) => a.petId && a.readyForAdoption);
  if (ready.length === 0) return false;
  escortReadyPetsToLobby();
  return false;
}

function handleShelterIntake() {
  if (G.money < 500) return false;

  const petsInCare = G.agents.filter(a => a.petId).length;
  const readyCount = G.agents.filter(a => a.readyForAdoption).length;
  const maxPets = Math.max(3, Math.floor(G.totalRevenue / 2500) + 2);

  if (petsInCare >= maxPets && readyCount === 0) return false;

  const nextPet = peekNextIntakePet();
  if (!nextPet) return false;

  const check = canIntakePet(nextPet);
  if (!check.ok) return false;

  const reserve = Math.max(1500, G.metrics.dailyCosts * 3);
  if (G.money < 500 + reserve) return false;

  const agent = intakeRandomPet();
  if (agent) {
    showToast(`🤖 AI Director intaked ${agent.name}!`);
    return true;
  }
  return false;
}
