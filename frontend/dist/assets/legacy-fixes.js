(function () {
  ...
  /**
   * Facebook embed facade: the page plugin iframe pulls heavy Facebook
   * bundles that flood the console with Facebook's own internal errors
   * (and ad-blocker filter noise). Show a click-to-load card instead;
   * the feed loads only for visitors who ask for it.
   */
  function monroeGateFacebookEmbed(band) {
    if (!band || band.dataset.fbGate === '1') {
      return;
    }
    band.dataset.fbGate = '1';

    var viewport = band.querySelector('.home-editable-social-embed__viewport');
    var loader = band.querySelector('.home-editable-social-embed__loader');
    if (!viewport) {
    ...
    facade.className = 'home-editable-social-embed__facade';
    var title = document.createElement('p');
    title.className = 'home-editable-social-embed__facade-title';
    title.textContent = 'HSMC on Facebook';
    var note = document.createElement('p');
    note.className = 'home-editable-social-embed__facade-note';
    note.textContent = 'Load the live feed to see our latest posts, events, and adoptable pets.';
    var btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'home-editable-social-embed__facade-btn';
    btn.textContent = 'Load Facebook feed';
