...
import { onHabitatPlaced } from './ui/tutorial.js';
...
export function handleBuildClick(sx, sy) {
  ...
  if (corridorMode) {
    ...
    sfxError();
    if (!isOnFloor(tx, ty)) {
      showToast('Paint corridors on the green buildable floor only.');
    } else if (tileMap[ty]?.[tx] !== 0) {
      showToast('That tile is occupied - corridors need empty floor.');
    } else {
      showToast('Cannot place corridor here.');
    }
    return true;
  }
  ...
  if (!canPlaceRoom(rx, ry, w, h)) {
    ...
    const tileMap = getTileMap();
    let onFloor = true;
    for (let dy = 0; dy < h && onFloor; dy++) {
      for (let dx = 0; dx < w; dx++) {
        if (!isOnFloor(rx + dx, ry + dy)) { onFloor = false; break; }
      }
    }
    if (!onFloor) {
      showToast('Place rooms on the green buildable area.');
    } else if (!isAdjacentToCorridor(rx, ry, w, h)) {
      showToast('Touch a tan corridor tile on one edge - rooms need walkway access.');
    } else {
      showToast('Cannot place here - another room may be in the way.');
    }
    return true;
  }
  ...
  if (room) {
    ...
    if (['kennel', 'cattery', 'playyard'].includes(selectedRoomType)) {
      onHabitatPlaced(selectedRoomType);
    }
