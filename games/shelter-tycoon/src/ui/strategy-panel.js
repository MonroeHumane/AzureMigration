...
function buildShelterStats() {
  ...
  const dailyRev = G.metrics.dailyAdoptionRevenue || G.metrics.dailyRevenue || 0;
  const outreach = G.marketingBudget || 0;

  const builtRooms = getRoomInstances();
  let habitatHtml = '';
  for (const key of ['kennel', 'cattery', 'playyard', 'vetbay', 'lobby']) {
    const room = builtRooms.find(r => r.typeKey === key);
    if (!room) continue;
    const petsHere = G.agents.filter(a => a.petId && a.role?.office === key).length;
    const activePlans = G.projects.filter(p => p.isCarePlan && p.targetOffice === key && p.state === 'in_progress').length;
    habitatHtml += `
      <div class="strategy-row">
        <div class="strategy-row-main">${room.type.icon} ${room.type.name}</div>
        <div class="strategy-row-stats">${petsHere} pets · ${activePlans} active care plan${activePlans !== 1 ? 's' : ''}</div>
      </div>
    `;
  }

  return {
    summary: `Pets in care: ${petsInCare} · Ready: ${ready} · Adopted: ${adoptions} · Today: $${Math.round(dailyRev).toLocaleString()} donations`,
    outreachNote: outreach > 0
      ? `Outreach budget $${outreach}/day draws more adopters to the lobby.`
      : 'Raise outreach budget in Build panel to attract more adopters.',
    habitats: habitatHtml || '<div class="strategy-empty">Build kennels or a cattery to start intake.</div>',
  };
}

function getShelterBottleneck() {
  const petsInCare = G.agents.filter(a => a.petId).length;
  if (petsInCare === 0) return { state: 'blocked', text: 'No pets in care. Build a habitat and intake from the Monroe catalog.' };
  const ready = G.agents.filter(a => a.readyForAdoption).length;
  if (ready === 0) return { state: 'warn', text: 'Care plans in progress - complete phases to ready pets for adoption.' };
  if ((G.adoptersToday || 0) === 0 && G.reputation < 35) {
    return { state: 'warn', text: 'Low reputation - boost outreach or improve adopter visits.' };
  }
  return { state: 'ok', text: 'Adoption pipeline healthy. Keep care plans moving and lobby welcoming.' };
}

function getAdoptionMixSummary() {
  const recent = (G.completedLog || []).filter(c => c.source === 'adoption').slice(0, 10);
  if (recent.length === 0) return 'Recent adoptions: none yet - finish care plans and welcome adopters.';
  const totalDonations = recent.reduce((s, c) => s + (c.pay || 0), 0);
  return `Last ${recent.length} adoptions · $${Math.round(totalDonations).toLocaleString()} in donations`;
}
...
export function updateStrategyPanel() {
  ...
  if (G.companyType === 'animal_shelter') {
    const shelter = buildShelterStats();
    const bottleneck = getShelterBottleneck();
    const bottleneckColor = bottleneck.state === 'ok'
      ? 'var(--success)'
      : bottleneck.state === 'warn' ? 'var(--warning)' : '#e09090';
    const adoptionMix = getAdoptionMixSummary();

    strategyEl.innerHTML = `
      <div class="strategy-card">
        <h4>Shelter Identity</h4>
        <div class="strategy-business">${identity.title}</div>
        <div class="strategy-note">${identity.subtitle}</div>
      </div>

      <div class="strategy-card">
        <h4>Adoption Pipeline</h4>
        <div class="strategy-note">${shelter.summary}</div>
        <div class="strategy-note">${shelter.outreachNote}</div>
        <div class="strategy-note" style="margin-top:6px;color:${bottleneckColor}">Diagnosis: ${bottleneck.text}</div>
        <div class="strategy-note" style="margin-top:4px">${adoptionMix}</div>
        ${shelter.habitats}
      </div>

      <div class="strategy-card">
        <h4>Facility Upgrades</h4>
        ${getTechTreeRows()}
      </div>
    `;
  } else {
