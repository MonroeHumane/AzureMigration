  ...
  switch (tipKey) {
    ...
    case 'funnel': {
      if (G.companyType === 'animal_shelter') {
        ...
        const mktBudget = G.marketingBudget || 0;
        return `
          <div class="hud-popover-title">🐾 Shelter KPIs</div>
          <div class="hud-popover-desc">Track pets in care, adoption readiness, and daily donor traffic.</div>
          ${row('🐾 Pets in Care', petsInCare)}
          ${row('✅ Ready for Adoption', ready, ready > 0 ? '#50c878' : '')}
          ${row('👥 Adopters Today', G.adoptersToday || 0)}
          ${divider()}
          ${row('💵 Daily Donations', `$${Math.round(m.dailyAdoptionRevenue || m.dailyRevenue || 0).toLocaleString()}`, '#50c878')}
          ${row('🏠 Total Adopted', G.adoptedCount || 0)}
          ${mktBudget > 0 ? row('Outreach Budget', `$${mktBudget}/day`) : '<div class="hud-popover-desc" style="margin:4px 0 0;font-size:10px;color:var(--text-faint)">💡 Raise outreach budget to attract more adopters</div>'}
        `;
      }
