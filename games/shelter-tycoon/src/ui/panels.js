...
import { getOfficeCategoriesForCompany, getReputationPremium, hasProductGrowth, getConsultingPremium, updateShelterMetrics } from '../economy.js';
import { Project } from '../project.js';
import { rollCandidatesForRole, getCandidatesForRole } from '../intake.js';
import { startTeamBuilding } from '../simulation.js';
import { showToast } from './toast.js';

function isShelterMode() {
  return G.companyType === 'animal_shelter';
}
...
function getNoProjectsDiagnosis() {
  if (isShelterMode()) {
    ...
    if (petsInCare === 0) {
      return 'No care plans yet - build a habitat and intake your first pet.';
    }
    const ready = G.agents.filter(a => a.readyForAdoption).length;
    if (ready > 0) {
      return `${ready} pet${ready !== 1 ? 's' : ''} ready for adoption - adopters will visit the lobby.`;
    }
    const stalled = G.projects.filter(p => p.isCarePlan && p.stalled).length;
    if (stalled > 0) {
      return `${stalled} care plan${stalled !== 1 ? 's' : ''} stalled - build the required habitat room.`;
    }
    return 'Care plans in progress - complete all phases to ready pets for adopters.';
  }
...
export function updateAgentPanel() {
  ...
  let statusText = a.inMeeting ? '🎯 In Meeting'
    ...
    : a.state;
  if (G.companyType === 'animal_shelter' && a.roleKey === 'ceo' && a.state === 'idle' && !a.task && !a._servingVisitor) {
    const ready = G.agents.filter((ag) => ag.petId && ag.readyForAdoption).length;
    const adopters = (G.visitors || []).filter((v) =>
      (v.type === 'adopter' || v.type === 'client') && v.state !== 'gone' && v.state !== 'leaving',
    ).length;
    if (ready > 0 && adopters > 0) {
      statusText = `🏠 Greet adopters (${adopters} waiting, ${ready} ready)`;
    } else if (ready > 0) {
      statusText = `🐾 ${ready} pet${ready !== 1 ? 's' : ''} ready - adopters will visit the lobby`;
    }
  }
...
export function updateUI() {
  ...
  // Projects / Care Plans tab
  const pl = document.getElementById('project-list');
  pl.innerHTML = G.projects.map(p => {
    const progress = p.progress * 100;
    const isCare = p.isCarePlan;
    const workerSuffix = p.assignedAgents?.length ? ` · ${p.assignedAgents.length} worker${p.assignedAgents.length !== 1 ? 's' : ''}` : '';
    const phase = p.state === 'in_progress'
      ? (isCare ? p.getPhaseLabel() : `${p.currentPhase}${workerSuffix}`)
      : (p.stalled ? '<span style="color:#e05050">Needs habitat!</span>' : 'Queued');
    const urgent = !isCare && p.deadlineUrgent;
    ...
    const payLine = isCare
      ? `<span class="pay" style="color:var(--text-dim)">${Math.round(progress)}%</span>`
      : `<span class="pay" style="color:${p.pay < 0 ? '#e05050' : ''}">${p.pay < 0 ? '-$' + Math.abs(p.pay).toLocaleString() : '$' + p.pay.toLocaleString()}</span>`;
    const metaLine = isCare
      ? `<div style="margin-top:5px;font-size:10px;color:var(--text-faint)">${p.petAgent?.name || 'Pet'} · ${p.template.office}</div>`
    ...
    return `<div class="project-card"${borderStyle}>
      <div class="row"><span class="title">${p.template.icon} ${p.name}</span>${payLine}</div>
      ${metaLine}
  ...
  const alignHud = document.querySelector('[data-hud-tip="alignment"]');
  if (alignHud) alignHud.style.display = isShelterMode() ? 'none' : '';

  // HUD funnel / shelter KPIs
  if (isShelterMode()) {
    ...
    const petsInCare = G.agents.filter(a => a.petId).length;
    const ready = G.agents.filter(a => a.readyForAdoption).length;
    if (hudVisitors) hudVisitors.textContent = petsInCare;
    if (hudLeads) hudLeads.textContent = ready;
    if (hudProjects) hudProjects.textContent = G.adoptersToday || 0;
    if (hudAcv) hudAcv.textContent = '$' + Math.round(G.metrics.dailyAdoptionRevenue || G.metrics.dailyRevenue || 0).toLocaleString();
  } else {
    const hudVisitors = document.getElementById('hud-visitors');
    const hudLeads = document.getElementById('hud-leads');
    const hudProjects = document.getElementById('hud-projects');
    ...
    financeSummary.innerHTML = `
      ...
      <div style="font-size:12px">
        ...
        <div style="display:flex;gap:12px;padding:6px 0;color:var(--text-faint);font-size:11px;justify-content:flex-end">
          <span>${isShelterMode() ? '🐾' : '👥'} ${isShelterMode() ? G.agents.filter(a => a.petId).length : G.agents.length} ${isShelterMode() ? 'pets' : 'staff'}</span>
          <span>📁 ${G.projects.filter(p => p.state === 'in_progress').length} ${isShelterMode() ? 'care plans' : 'active'}</span>
        </div>
        ${G.debt > 0 ? `
