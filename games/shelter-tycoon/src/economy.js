...
import {
  ...
  GROWTH_MODELS, PET_DAILY_UPKEEP,
} from './config.js';
import { getRoomInstances, countRoomsByType } from './map.js';
import { G } from './game.js';
import { hasClinicBranch } from './progression.js';
import {
...
// Update shelter KPI metrics (HUD + analytics)
export function updateShelterMetrics() {
  const pets = G.agents.filter(a => a.petId);
  G.metrics.petsInCare = pets.length;
  G.metrics.readyForAdoption = pets.filter(a => a.readyForAdoption).length;
  G.metrics.dailyAdoptionRevenue = G.metrics.dailyRevenue || 0;
}

// Daily costs calculation - shelter: room rent + per-pet daily upkeep
export function calculateDailyCosts() {
  let roomCost = 0;
  const rooms = getRoomInstances();
  for (let i = 0; i < rooms.length; i++) {
    const cost = ROOM_COSTS[rooms[i].typeKey] || 0;
    if (rooms[i].typeKey === 'lobby' && G.day <= (ECONOMY.grace_period_days || 7)) {
      continue;
    }
    roomCost += cost;
  }
  ...
  let petUpkeep = 0;
  let medicalUpkeep = 0;
  const vetDiscount = countRoomsByType('vetbay') > 0 ? (hasClinicBranch() ? 0.4 : 0.2) : 0;

  for (const agent of G.agents) {
    if (agent.roleKey === 'ceo' || !agent.petId) continue;
    const base = PET_DAILY_UPKEEP[agent.roleKey] ?? AGENT_SALARY[agent.roleKey] ?? 12;
    petUpkeep += base;
    const onMedical = agent.task?.isCarePlan && agent.task?.currentPhase === 'medical';
    if (onMedical) {
      medicalUpkeep += Math.round(base * 0.5);
    }
  }

  if (vetDiscount > 0) {
    medicalUpkeep = Math.round(medicalUpkeep * (1 - vetDiscount));
  }

  const facilityCost = G.agents.some(a => a.petId) ? getPerksCost() + getWorkspaceCost() : 0;
  const outreachSpend = G.marketingBudget || 0;
  const freeWorkerDiscount = getFreeWorkerSavings();
  const totalCost = Math.max(0, roomCost + petUpkeep + medicalUpkeep + outreachSpend + facilityCost - freeWorkerDiscount);
  G.metrics.dailyCosts = totalCost;
  return totalCost;
}

// Update analytics level based on shelter operations
export function updateAnalyticsLevel() {
  const pets = G.agents.filter(a => a.petId).length;
  const adoptions = G.adoptedCount || 0;
  if (pets === 0 && adoptions === 0) {
    G.analyticsLevel = 0;
  } else if (adoptions < 3) {
    G.analyticsLevel = 1;
