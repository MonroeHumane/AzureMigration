// ═══════════════════════════════════════════════════════════════
//  SHELTER TYCOON - Game State Singleton
// ═══════════════════════════════════════════════════════════════
...
export const G = {
  ...
  companyType: 'animal_shelter',

  // Monroe / WordPress integration (playerNickname = Adoptédex username_slug)
  playerNickname: null,
  ...
  unlockedRooms: new Set(['lobby', 'kennel', 'cattery']),
  ...
  metrics: {
    ...
    petsInCare: 0,
    readyForAdoption: 0,
    dailyAdoptionRevenue: 0,
    retentionRate: 60,
    referralRate: 0,
  },

  checkWinCondition() {
    if ((this.totalRevenue >= WIN_REVENUE || (this.adoptedCount || 0) >= 25) && !this.gameWon) {
      this.gameWon = true;
      this.gameOver = true;
      trackEvent('game-win', { day: this.day, company: this.companyType, revenue: this.totalRevenue, adoptions: this.adoptedCount });
      return 'win';
    }
  ...
  reset() {
    this.companyType = 'animal_shelter';
    this.playerNickname = null;
    this.playerDisplayName = null;
    this.adoptedPetIds = [];
    this.adoptedCount = 0;
    this.lastAdoptionDay = 0;
    this.adoptersToday = 0;
    this.shelterCatalog = [];
    this.availablePets = [];
    this.lastSaveDay = 0;
    this.savePending = false;
    this.money = START_MONEY;
    this.unlockedRooms = new Set(['lobby', 'kennel', 'cattery']);
    this.totalRevenue = 0;
