...
function drawCorridorTile(ctx, s, tw, th, x, y) {
  const check = (x + y) % 2 === 0;
  // High-contrast tan walkways (distinct from green buildable grass)
  ctx.fillStyle = check ? '#d4a96a' : '#c49558';
  fillDiamond(ctx, s, tw, th);

  ctx.fillStyle = check ? 'rgba(255, 248, 235, 0.55)' : 'rgba(120, 72, 32, 0.12)';
  const inset = tw * 0.1;
  ctx.beginPath();
  ctx.moveTo(s.x, s.y - th / 2 + inset / 2);
  ctx.lineTo(s.x + tw / 2 - inset, s.y);
  ...
  // Orange edge accent so corridors read as paths
  ctx.strokeStyle = 'rgba(232, 104, 32, 0.35)';
  ctx.lineWidth = 1.2;
  ctx.beginPath();
  ...
  ctx.fillStyle = 'rgba(232, 104, 32, 0.28)';
  ctx.beginPath();
  ...
  for (let y = 0; y < MAP_H; y++) {
    for (let x = 0; x < MAP_W; x++) {
      ...
      if (tileType === 0) {
        const m = (x + y) % 2 === 0 ? 0 : 4;
        if (onFloor) {
          ctx.fillStyle = `rgb(${126 + m},${178 + m},${104 + m})`;
        } else {
          ctx.fillStyle = `rgb(${118 + m},${168 + m},${98 + m})`;
        }
        fillDiamond(ctx, s, tw, th);

        if (onFloor && floorMask) {
          ...
          if (edgeRight || edgeDown || edgeLeft || edgeUp) {
            ctx.strokeStyle = 'rgba(61, 154, 69, 0.45)';
            ctx.lineWidth = 1;
            ctx.beginPath();
            if (edgeRight) { ctx.moveTo(s.x, s.y); ctx.lineTo(s.x + tw / 2, s.y + th / 2); }
            if (edgeDown)  { ctx.moveTo(s.x + tw / 2, s.y); ctx.lineTo(s.x, s.y + th / 2); }
            if (edgeLeft)  { ctx.moveTo(s.x, s.y); ctx.lineTo(s.x - tw / 2, s.y + th / 2); }
