// ═══════════════════════════════════════════════════════════════
//  SHELTER TYCOON - Simulation Tick Orchestrator
// ═══════════════════════════════════════════════════════════════

import { ECONOMY, PROJECT_TEMPLATES, TUTORIAL_HINTS, COMPANY_TYPES, OFFICE_TYPES, AGENT_ROLES, AGENT_SALARY, SPEECH_RAISE, SPEECH_QUIT, ALIGNMENT, SPEECH_QUIT_MISALIGNED, SPEECH_ALIGNED, SPEECH_TEAM_BUILDING, GROWTH_MODELS, ADOPTION_DONATION, WIN_REVENUE } from './config.js';
...
import { syncRoomUnlocks, getRoomLabel, hasSpecializeBranch, hasEnrichmentBranch, hasOutreachBranch } from './progression.js';
import {
...
import { hasHRStaff } from './intake.js';
import { onPetAdoptedOut } from './monroe-api.js?v=13';
...
// ─── Care Plan auto-progress (volunteer/staff abstracted) ──
function getCarePlanSpeed(plan) {
  let speed = 0.32;
  if (countRoomsByType('breakroom') > 0) speed += 0.08;
  if (countRoomsByType('meeting') > 0) speed += 0.06;
  if (plan.currentPhase === 'socialization' && countRoomsByType('playyard') > 0) {
    speed += hasEnrichmentBranch() ? 0.45 : 0.22;
  }
  if (plan.currentPhase === 'medical' && countRoomsByType('vetbay') > 0) speed += 0.18;
  return speed;
}

function updateCarePlans(dt) {
  for (const p of G.projects) {
    if (!p.isCarePlan || p.state === 'done') continue;
    if (!findRoomByType(p.targetOffice)) {
      p.stalled = true;
      continue;
    }
    p.stalled = false;
    if (p.state === 'waiting') p.state = 'in_progress';

    p.phaseProgress += (dt / (p.phaseTime * 60)) * getCarePlanSpeed(p);

    if (p.phaseProgress >= 1) {
      const quality = 0.5 + Math.random() * 0.3;
      const finished = p.advancePhase(quality);
      if (finished) {
        if (p.petAgent) {
          p.petAgent.readyForAdoption = true;
          p.petAgent.task = null;
          G.completedTriggers.add('first_ready');
          escortReadyPetToLobby(p.petAgent);
        }
        sfxProjectComplete();
        showToast(`🎉 ${p.petAgent?.name || 'Pet'} is ready for adoption!`);
        G.uiDirty = true;
      } else {
        showToast(`📋 ${p.petAgent?.name || 'Pet'}: ${p.getPhaseLabel()} started`);
        if (p.petAgent) p.petAgent.task = p;
      }
    }
  }
  G.projects = G.projects.filter(p => !(p.isCarePlan && p.state === 'done'));
}

async function completeAdoption(petAgent) {
  if (!petAgent?.petId) return false;

  const donation = ADOPTION_DONATION;
  G.money += donation;
  G.totalRevenue += donation;
  G.dayRevenueAcc += donation;
  G.adoptedCount = (G.adoptedCount || 0) + 1;
  G.completedTriggers.add('first_adoption');
  G.lastAdoptionDay = G.day;
  G.metrics.officeRevenue.lobby = (G.metrics.officeRevenue.lobby || 0) + donation;

  G.completedLog.unshift({
    name: `${petAgent.name} adopted`,
    petId: petAgent.petId,
    pay: donation,
    quality: 1,
    time: G.gameTick,
    source: 'adoption',
  ...
  await onPetAdoptedOut(petAgent.petId, G.playerNickname);

  G.projects = G.projects.filter(p => p.petId !== petAgent.petId);
  G.agents = G.agents.filter(a => a !== petAgent);
  if (G.selectedAgent === petAgent) deselectAgent();

  const room = findRoomByType('lobby');
  if (room) {
  ...
  sfxMoney();
  showToast(
    `🏠 ${petAgent.name} found a home!<span class="toast-money">+$${donation.toLocaleString()}</span>`,
    { html: true }
  );
  G.uiDirty = true;
  return true;
...
function assignTasks() {
  for (const project of G.projects) {
    if (project.isCarePlan) continue;
...
function updateProjects(dt) {
    ...
    if (workers.length > 0) {
      ...
      if (p.phaseProgress >= 1) {
        ...
        if (p.advancePhase(avgQuality + collabBonus)) {
          ...
          // HR synergy removed - shelter uses Monroe intake catalog
  ...
  // Remove done projects (care plans removed in updateCarePlans)
...
function processDayCycle(dt) {
  ...
  if (G.dayTicks >= ECONOMY.day_ticks) {
    ...
    for (const a of G.agents) {
      a.daysSinceBreak++;
      if (a.roleKey === 'ceo' || a.petId) continue;
    ...
    // ─── Salary Negotiation & Quitting (staff only - not shelter pets) ───
    ...
    // Reputation maintenance: decay only when warranted (not a pet-only roster)
    const petOnlyRoster = G.agents.every(a => a.petId || a.roleKey === 'ceo');
    const daysSinceAdoption = G.lastAdoptionDay ? G.day - G.lastAdoptionDay : G.day;
    const shouldDecayRep = !petOnlyRoster && (daysSinceAdoption >= 7 || G.reputation > 70);
    if (shouldDecayRep) {
      G.reputation = Math.round(Math.max(0, G.reputation - 0.05));
    } else if (hasSupportStaffed()) {
    ...
    // Business-only level intro (shelter uses guided tutorial instead)
    if (G.companyType !== 'animal_shelter') {
    ...
    // Auto-save to WordPress at end of each day
    if (G.playerNickname && typeof G.scheduleSave === 'function') {
      G.scheduleSave();
    }
    ...
    if (result === 'win') {
      sfxWin();
      showToast(`🎉 Mission accomplished! 25 forever homes or $${WIN_REVENUE.toLocaleString()} in donations!`);
    } else if (result === 'need_loan') {
      G.gameSpeed = 0;
      showLoanModal();
    } else if (result === 'bankrupt') {
      sfxGameOver();
      showToast('Shelter operations suspended - funds exhausted.');
    }
...
// ─── Adopter spawning (when pets are ready) ────────────────
function spawnAdopters(dt) {
  if (G.gameOver) return;

  const readyPets = G.agents.filter(a => a.readyForAdoption && a.petId);
  if (readyPets.length === 0) return;

  const outreachMult = hasOutreachBranch() ? 1.35 : 1.0;
  const lobbyBonus = findRoomByType('lobby') ? 1.2 : 0.8;
  const marketingBoost = ((G.marketingBudget || 0) / 100) * 0.001;
  const readyBoost = readyPets.length >= 2 ? 0.004 : 0;
  const rate = ((G.reputation / 100) * 0.006 * outreachMult * lobbyBonus * readyPets.length)
    + marketingBoost + readyBoost;
  G.spawnTimer += dt * rate;

  while (G.spawnTimer >= 1 && G.visitors.length < getMaxVisitors()) {
    G.spawnTimer -= 1;
    spawnVisitor('adopter', 'lobby');
    G.completedTriggers.add('first_project');
  }
}

// ─── Legacy project spawn (disabled for shelter - kept for compat) ──
function spawnProjects(dt) {
  spawnAdopters(dt);
  if (G.gameOver) return;

  // Starter flow: first intake hint only
  if (!G.starterProjectsGiven && G.agents.some(a => a.petId)) {
    G.starterProjectsGiven = true;
    sfxFirstClients();
    showToast('🐾 Care plans are underway! Finish phases to ready pets for adopters.');
  }

  return; // Skip legacy client funnel spawning
...
const FIRST_HIRE_HINTS = {
  animal_shelter: 'Intake your first pet from the Monroe catalog! Open Intake and choose an arrival.',
};
const FIRST_HIRE_DEFAULT = 'Build Kennels or a Cattery, then intake pets from the catalog.';
...
function updateTutorialHints() {
  ...
  if (G.companyType === 'animal_shelter') {
    ...
    if (G.completedTriggers.has('habitat_ready') && !G.shownHints.has('first_room')) {
      showTutorialHint('first_room');
      return;
    }
    if (G.completedTriggers.has('first_hire') && !G.shownHints.has('first_intake')) {
      showTutorialHint('first_intake');
      return;
    }
    ...
    if (G.completedTriggers.has('first_ready') && !G.shownHints.has('first_ready')) {
      showTutorialHint('first_ready');
      return;
    }
    if (G.completedTriggers.has('first_adoption') && !G.shownHints.has('first_adoption')) {
      showTutorialHint('first_adoption');
      return;
    }
    return;
  }
...
function startStandup(meetingRoom) {
  ...
  // Send available agents - exclude pets; those serving visitors stay behind
  const allAgents = G.ceo ? [G.ceo, ...G.agents.filter(a => a !== G.ceo && !a.petId)] : G.agents.filter(a => !a.petId);
...
export function startTeamBuilding() {
  ...
  // Staff agents join the meeting - exclude pets; those serving visitors stay put
  for (const a of G.agents) {
    if (a.roleKey === 'ceo' || a.petId) continue;
    if (a._servingVisitor) continue; // don't abandon customers!
    a.inMeeting = true;
...
// ─── Shelter adoption flow (lobby meet-and-greet) ───────────
function isShelterStaffAgent(agent) {
  if (!agent || agent.petId) return false;
  if (String(agent.roleKey || '').startsWith('pet_')) return false;
  return true;
}

function agentCanServeVisitors(agent) {
  return agent
    && agent.state === 'idle'
    && !agent.task
    && !agent.inMeeting
    && !agent._servingVisitor
    && agent.energy > 0.1;
}

/** Walk adoption-ready pets to the lobby so adopters can meet them. */
export function escortReadyPetToLobby(petAgent) {
  if (G.companyType !== 'animal_shelter' || !petAgent?.petId || !petAgent.readyForAdoption) return;
  if (petAgent._servingVisitor) return;

  const lobby = findRoomByType('lobby');
  if (!lobby) return;

  const px = Math.round(petAgent.x);
  const py = Math.round(petAgent.y);
  const inLobby = px >= lobby.x && px < lobby.x + lobby.w && py >= lobby.y && py < lobby.y + lobby.h;
  if (inLobby) return;

  if (petAgent._lobbyEscortCooldown > 0) {
    petAgent._lobbyEscortCooldown -= 1;
    return;
  }

  if (petAgent.moveToRoom(lobby.id)) {
    petAgent._lobbyEscortCooldown = 90;
  }
}

export function escortReadyPetsToLobby() {
  if (G.companyType !== 'animal_shelter') return;
  for (const agent of G.agents) {
    if (agent.petId && agent.readyForAdoption) {
      escortReadyPetToLobby(agent);
    }
  }
}

function findAgentForVisitor(visitor) {
  if (G.companyType === 'animal_shelter') {
    const staffReady = (a) => isShelterStaffAgent(a) && agentCanServeVisitors(a);
    if (visitor.type === 'adopter' || visitor.type === 'client') {
      const director = G.agents.find((a) => a.roleKey === 'ceo' && staffReady(a));
      if (director) return director;
      return G.agents.find(staffReady);
    }
  }

  const role = visitor.requestedRole;
  if (role) {
    const roleAgent = G.agents.find((a) =>
      a.role?.office === role && agentCanServeVisitors(a),
    );
    if (roleAgent) return roleAgent;
  }
  return G.agents.find((a) => agentCanServeVisitors(a));
}

function releasePetFromAdopterDuty(visitor) {
  if (!visitor.assignedAgent?.petId) return;
  if (visitor.type !== 'adopter' && visitor.type !== 'client') return;
  visitor.assignedAgent._servingVisitor = null;
  visitor.assignedAgent = null;
  visitor.needsAgent = true;
  if (visitor.state === 'agent_arriving') {
    visitor.state = 'seated_waiting';
  }
}

// ─── Visitors ───────────────────────────────────────────────
const PATIENCE_DRAIN = { adopter: 0.00035, client: 0.00035, candidate: 0.0002, walkin: 0.0004, customer: 0.0003 };
...
function spawnVisitor(type, targetRoom) {
  ...
  G.visitors.push(v);
  if (type === 'adopter') G.adoptersToday = (G.adoptersToday || 0) + 1;
  return v;
}
...
function tickVisitors(dt) {
  ...
  // 2. Agent dispatch - find idle staff to send to visitors who need them
  for (const v of G.visitors) {
    releasePetFromAdopterDuty(v);

    if (!v.needsAgent || v.assignedAgent) continue;
    if (v.state !== 'seated_waiting') continue;

    const matchingAgent = findAgentForVisitor(v);
...
function applyVisitorOutcome(v) {
  ...
  if (v.type === 'adopter' || v.type === 'client') {
    if (satisfied) {
      const lobby = findRoomByType('lobby');
      const readyPet = G.agents.find((a) => {
        if (!a.readyForAdoption || !a.petId) return false;
        if (!lobby) return true;
        const px = Math.round(a.x);
        const py = Math.round(a.y);
        return px >= lobby.x && px < lobby.x + lobby.w && py >= lobby.y && py < lobby.y + lobby.h;
      }) || G.agents.find((a) => a.readyForAdoption && a.petId);
      if (readyPet) {
        completeAdoption(readyPet);
        G.reputation = Math.min(100, G.reputation + (verySatisfied ? 2 : 1));
        G.visitorStats.totalServed++;
        return;
      }
      G.reputation = Math.min(100, G.reputation + 0.5);
      showToast('💬 Adopter loved the visit - no pets ready yet.');
    } else if (angry) {
...
export function simulationTick(dt) {
  ...
  for (const room of builtRooms) {
    ...
    if (['kennel', 'cattery', 'playyard'].includes(room.typeKey)) {
      G.completedTriggers.add('habitat_ready');
    }
    G.uiDirty = true;
  }
